...
- **Inbox + subfolders:** When the user asks to **check the inbox / mailbox / unread / latest mail** without naming one folder, call **`microsoft_mail_list_inbox_tree`** first (MCP tool — covers Inbox root + first-level subfolders in one round). Do **not** rely on **`microsoft_mail_list_messages`** alone for that case (root Inbox only). For deeper nesting only, use ``GET /me/mailFolders/{id}/childFolders`` then messages per id.

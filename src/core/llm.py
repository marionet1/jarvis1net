...
- **Inbox = root + subfolders:** When the user asks to **check the inbox / mailbox / unread / latest mail** without naming a single folder, assume scope is **Inbox plus every subfolder under Inbox** (Graph does **not** include subfolder messages in ``/me/mailFolders/inbox/messages`` alone). Use ``GET /me/mailFolders/inbox/childFolders?$select=id,displayName,unreadItemCount`` (no deep expand), then query messages or unread counts **per folder** (inbox id + each child ``id``) and **combine** results in the answer. Only restrict to “top-level Inbox only” if the user explicitly says so.
